...
* `participants.tsv`: A TSV file listing the participants and their metadata. **`nf-mouse_flow` requires the participants' age to be supplied within this file, using `age` as the column name. If not available, the pipeline will return an error at runtime.**
...
In order for `nf-mouse_flow` to properly read the `participants.tsv` file, columns need to be **tab-separated**. If the pipeline returns an error regarding age not being correctly entered, this should be the **first validation step** to take (e.g., ensure a single tab is separating each of your columns).
:::
