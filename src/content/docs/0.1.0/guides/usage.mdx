...
![nf-mouse pipeline schema](../../../../assets/0.1.0/nf-mouse-schema.png)
...
<Steps>
  1. **`-pre_qc`:**
     ...
     :::caution
     **If you want to use the output of the **pre QCed** data you must add --use_preqc. See [parameters section](/nf-mouse/0.1.0/guides/parameters).**
     :::
...
<Tabs>
  <TabItem label="Command">
    ```bash
    nextflow run ~/s/nf-mouse/main.nf --input <input_directory> --outdir ./results --lean_output true -profile docker,pre_qc --use_preqc -resume -with-report <report_name>.html
    nextflow run scilus/nf-mouse -r 0.1.0 --input <input_directory> --outdir ./results --use_preqc true -profile docker,pre_qc -resume
    ```
  </TabItem>
</Tabs>
...
:::note
More parameters can be tuned to your own usage, for a concise list of the most common parameters, you can run `nextflow run scilus/nf-mouse -r 0.1.0 --help`. Otherwise, for the complete list of parameters, please refer to the [parameters section](/nf-mouse/0.1.0/guides/parameters) of the documentation.
:::
...
<FileTree>
  ...
  * nf-mouse-v0.1.0/            # Finished results in specified location (defined with --outdir)
    * sub-01
        * anat/
        * dwi/
        * stats/
