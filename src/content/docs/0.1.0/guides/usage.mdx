...
description: How to use nf-mouse_flow
...
![nf-mouse_flow pipeline schema](../../../../assets/0.1.0/nf-mouse_flow-schema.png)

### **Choosing a profile**

Some `nf-mouse_flow` core functionalities are accessed and selected using profiles and arguments. 
...
<Steps>
  1. **`-pre_qc`:**
     ...
     By selecting this profile, 

     :::caution
     **If you want to use the output of the pre QCed data you must add --use_preqc.
     :::
...
:::note[Running the pre_qc]
For example, to perform the pre_qc and apptainer, users should select `-profile pre_qc,apptainer`.
:::
...
<Tabs>
  <TabItem label="Command">
    ```bash
    nextflow run scilus/nf-mouse_flow -r 0.1.0 --input <BIDS_directory> --outdir ./results --use_preqc true -profile docker,pre_qc -resume
    ```
  </TabItem>
</Tabs>

This will launch the pipeline with the `docker` configuration profile and will run the pre_qc processing steps. There are only 3 parameters that need to be supplied at runtime:

<Steps>
  1. **`--input`**: for the path to your BIDS directory

     For more details on how to organize your input folder, please refer to the [inputs section](/nf-mouse_flow/0.1.0/guides/inputs).
  ...
  2. **`--outdir`**: path to the output directory
     ...
     We do not specify a default for the output directory location to ensure that users have total control on where the output files will be stored, as it can quickly grow into a large number of files. The recommended naming would be something along the line of `nf-mouse_flow-v{version}` where `{version}` could be `0.1.0` for example.

  3. **`-profile`**: profile to be run and container system to use

     `nf-mouse_flow` processing steps was designed in profiles, giving users total control on which type of processing they want to make. One caveat is that users need to explicit tell which profile to run. This is done via the `-profile` parameter. To view the available processing profiles, please see [this section](#choosing-a-profile)

     :::note
     The `-profile` command line parameter is a core `nextflow` parameter, hence the preceding single dash. This means you can also specify nextflow specific profile side-by-side `nf-mouse_flow` processing profiles (e.g., the docker/apptainer selection).
     :::
...
:::note
More parameters can be tuned to your own usage, for a concise list of the most common parameters, you can run `nextflow run scilus/nf-mouse_flow -r 0.1.0 --help`. Otherwise, for the complete list of parameters, please refer to the [parameters section](/nf-mouse_flow/0.1.0/guides/parameters) of the documentation.
:::

:::caution[Running the pipeline in offline mode]
If the computer nodes you are planning to use do not have access to the internet, there is a few steps to go through to make sure the containers and templates are available during pipeline execution. Please head over to [running with no internet access section](/nf-mouse_flow/0.1.0/guides/nointernet).
:::
...
<FileTree>
  ...
  * nf-mouse_flow-v0.1.0/            # Finished results in specified location (defined with --outdir)
    * sub-01
        * anat/
        * dwi/
        * stats/
...
<Tabs>
  <TabItem label="Command">
    ```bash
    nextflow run scilus/nf-mouse_flow -r 0.1.0 -profile docker -params-file params.yaml
    ```
  </TabItem>
</Tabs>
...
First, go to the [scilus/nf-mouse_flow releases page](https://github.com/scilus/nf-mouse_flow/releases) and find the latest pipeline version - numeric only (eg. `0.1.0`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 0.1.0`. Of course, you can switch to another version by changing the number after the `-r` flag.
