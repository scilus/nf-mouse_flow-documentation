...
![nf-mouse_flow pipeline schema](../../../assets/nf-mouse_flow-schema.png)
...
1. **`-profile segmentation`:** 
   ...
   :::caution
   **A valid FreeSurfer license file is required for this profile. Specify the path to your license using [`--fs_license`](/nf-mouse_flow/guides/parameters#segmentation-options).** If you don't have a valid license, you can register for one [here](https://surfer.nmr.mgh.harvard.edu/registration.html).
   :::
...
1. **`--input`**: for the path to your BIDS directory
   ...
   For more details on how to organize your input folder, please refer to the [inputs section](/nf-mouse_flow/guides/inputs).
...
:::note
More parameters can be tuned to your own usage, for a concise list of the most common parameters, you can run `nextflow run scilus/nf-mouse_flow -r 0.1.0 --help`. Otherwise, for the complete list of parameters, please refer to the [parameters section](/nf-mouse_flow/guides/parameters) of the documentation.
:::

:::caution[Running the pipeline in offline mode]
If the computer nodes you are planning to use do not have access to the internet, there is a few steps to go through to make sure the containers and templates are available during pipeline execution. Please head over to [running with no internet access section](/nf-mouse_flow/guides/nointernet).
:::
